/**
 * RippleDistortion — ReactBits WebGL Liquid Displacement Effect
...
export function initRippleDistortion(mount, options = {}) {
  if (!mount) return () => {};
  ...
  try {
    ...
    mount.style.backgroundImage = `url("${src}")`;
  ...
  image.onerror = () => {
    if (!image.dataset.triedOg && (src.includes('hero.jpg') || src.includes('/hero.jpg'))) {
      image.dataset.triedOg = 'true';
      image.src = 'og-image.jpg';
      return;
    }
    // Fallback if not found
    image.src = 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&w=1200&q=85';
  };
  image.src = src;
...
export { initRippleDistortion };
export default initRippleDistortion;
