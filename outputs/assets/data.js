...
const MOCK_IDS = {
  artists: ['lumi', '48k', 'vule', 'monotone'],
  tracks: ['rel-lumi-1', 'rel-lumi-2', 'rel-48k-1', 'rel-vule-1', 'rel-monotone-1'],
  producers: [],
  caseStudies: [],
  submissions: []
};
      ...
      const mappedSupabaseArtists = artistsData
        ...
        .map(a => {
          ...
          return {
            ...
            instagram: a.instagram || stats.instagram || localCachedArtist.instagram || (a.socials?.instagram) || '',
            youtube: a.youtube || stats.youtube || localCachedArtist.youtube || (a.socials?.youtube) || '',
            tiktok: a.tiktok || stats.tiktok || localCachedArtist.tiktok || (a.socials?.tiktok) || '',
            facebook: a.facebook || stats.facebook || localCachedArtist.facebook || (a.socials?.facebook) || '',
            spotify: a.spotify || stats.spotify || localCachedArtist.spotify || (a.socials?.spotify) || '',
            socials: a.socials || stats.socials || localCachedArtist.socials || {
              instagram: a.instagram || stats.instagram || localCachedArtist.instagram || '',
              youtube: a.youtube || stats.youtube || localCachedArtist.youtube || '',
              tiktok: a.tiktok || stats.tiktok || localCachedArtist.tiktok || '',
              facebook: a.facebook || stats.facebook || localCachedArtist.facebook || '',
              spotify: a.spotify || stats.spotify || localCachedArtist.spotify || ''
            },
      ...
      if (Array.isArray(data.artists)) {
        ...
        const artistPromises = data.artists.filter(a => !MOCK_IDS.artists.includes(a.id)).map(async a => {
          const stats = {
            ...
            customLinks: a.customLinks || a.custom_links || [],
            socials: a.socials || {
              instagram: a.instagram || a.socials?.instagram || '',
              youtube: a.youtube || a.socials?.youtube || '',
              tiktok: a.tiktok || a.socials?.tiktok || '',
              facebook: a.facebook || a.socials?.facebook || '',
              spotify: a.spotify || a.socials?.spotify || ''
            },
            facebook: a.facebook || a.socials?.facebook || '',
            spotify: a.spotify || a.socials?.spotify || '',
            products: a.products || []
          ...
          const fullArtistPayload = {
            ...
            banking: a.banking || null,
            custom_links: a.customLinks || a.custom_links || [],
            socials: a.socials || stats.socials || {},
            stats: stats,
            updated_at: new Date().toISOString()
          };
