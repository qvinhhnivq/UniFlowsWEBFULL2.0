...
import { initPortalGlassSurfaces } from './glass-surface.js';

let appGradientWavesInstance = null;
let appDitherInstance = null;

export function initAppThemeAndShaders() {
  const savedTheme = localStorage.getItem('uniflows-theme') || localStorage.getItem('uniflows-portal-theme') || 'dark';
  applyAppTheme(savedTheme);

  document.querySelectorAll('#theme-toggle-btn, .theme-toggle-btn, #mobile-theme-toggle-btn, #top-nav-theme-slider').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      localStorage.setItem('uniflows-portal-theme', nextTheme);
    });
  });
...
export function applyAppTheme(theme) {
  ...
  if (isDark) {
    ...
    if (appGradientWavesInstance?.stop) {
      appGradientWavesInstance.stop();
    }
    const ditherEl = document.querySelector('#portal-dither-bg');
    if (ditherEl && !appDitherInstance) {
      try {
        appDitherInstance = initDither(ditherEl, {
    ...
    if (appDitherInstance) {
      appDitherInstance.resize?.();
      appDitherInstance.start?.();
    }
  } else {
    document.documentElement.removeAttribute('data-theme');
    document.body.classList.remove('dark-mode');
    if (appDitherInstance?.stop) {
      appDitherInstance.stop();
    }
    const bgEl = document.querySelector('#portal-gradient-waves-bg');
    if (bgEl && !appGradientWavesInstance) {
      try {
        appGradientWavesInstance = initGradientWaves(bgEl, {
          horizonColor: '#f1edff',
          waveColor: '#FF9FFC',
          crestColor: '#FFFFFF',
          speed: 0.4,
    ...
    if (appGradientWavesInstance) {
      appGradientWavesInstance.setSize?.();
      appGradientWavesInstance.start?.();
    }
  }
...
// Auto init theme & shaders if backdrop exists
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      if (document.querySelector('#portal-dither-bg') || document.querySelector('#portal-gradient-waves-bg')) {
        initAppThemeAndShaders();
      }
    });
  } else {
    if (document.querySelector('#portal-dither-bg') || document.querySelector('#portal-gradient-waves-bg')) {
